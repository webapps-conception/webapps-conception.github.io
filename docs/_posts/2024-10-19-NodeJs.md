...
Nous avons utilisé quelques exemples de [GitHub](https://github.com/webapps-conception/nodejs-requests){:target="_blank"}, le référentiel open source de tous les codes, pour votre meilleure compréhension. Ici, nous effectuons une requête GET à l'aide de l'option client HTTP en appelant les données stockées à partir de l'API fictive [JSONPlaceholder](https://github.com/webapps-conception/nodejs-requests/blob/master/JSONPlaceholder/users){:target="_blank"}. en réponse à la demande complétée, il enverra les données des dix utilisateurs ainsi que leurs noms et identifiants. dans cet article, nous avons mentionné différents types d'exemples montrant les appels basés sur les rappels, les promesses et les asynchrones/attentes.
